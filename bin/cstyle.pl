...
my $foreachMacros = "json_object_foreach|json_array_foreach";
...
line: while (<$filehandle>) {
    ...
    s/\s*NonNull\([0-9, ]+\)//; # Strip the word NonNull
    ...
    if (/\t/) {
        err("tab character detected; please set your \$EDITOR to use spaces");
    }
    if (/\t +\t/) {
    ...
    if (/^[A-Z]\w*\(/ && !/DfDeclFieldValueArray/ && !/KeyValueDeclBuffer/ &&
	!/XdbDeclValueDesc/ && !/XdbDeclValueArray/){
        err("Functions must be camelCase");
    }
...
#        }
    ...
    if (/\/\/\S/ && !/\/\/!/ && !/https?:\/\// && /[^\/\n]/) {    # C++ comments
        err("missing blank after C++ start comment");
    }
    if (/\/\/!\S/) {    # doxygen comments
        err("missing blank after doxygen start comment");
    }
    ...
    # Disabled by kkochis. Matches if (foo ||  // comment about foo
    #if (/\S   *(&&|\|\|)/ || /(&&|\|\|)   *\S/) {
    #    err("more than one space around boolean operator");
    #}
    if (/\b(for|if|while|switch|case)\(/) {
    ...
    if (/\w\s\(/) {
        ...
        s/\b($foreachMacros|for|if|while|switch|return|case|try|catch|sizeof)\s\(/XXX(/g;
        s/#elif\s\(/XXX(/g;
        s/^#define\s+\w+\s+\(/XXX(/;
        # do not match things like "void (*f)();"
        # or "typedef void (func_t)();"
        s/\w\s\(+\*/XXX(*/g;
        s/\b($typename|void|new)\s+\(+/XXX(/og;
        if (/\w\s\(/) {
...
#        }
    ...
    if (/^(\w+(\s|\*)+)+\w+\(.*\)(\s|)*$/ &&
        !/^(extern|static|operator)\b/) {
        err("return type of function not on separate line");
    }
    if (/^#define /) {
        # err("#define followed by space instead of tab");
    }
# No longer have to enforce parens after return because return is not a function
...
# This trips over false positives like array initialization in constructors
# E.g. Gvm.cpp:20 Gvm::Gvm() : targets_{NULL} {}
#    if (/\S{/ && !/{{/ && !/\({/) {
