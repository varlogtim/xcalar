...
# reverseMap = {typeMap[k]:k for k in typeMap.keys()}
...
class QueryBuilder:
    def __init__(self,
        ...
        self.query = []
        self.counter = 1
        self.tempTablePrefix = tempTablePrefix
    ...
    def getTempTableName(self, next=False, table_in=None):
        ...
        if next:
            self.counter += 1
        return f"{self.tempTablePrefix}#{self.counter}"
    ...
    def XcalarApiSynthesize(self,
        ...
        op = {
            ...
            "args": {
                ...
                "dest":
                    table_out if table_out is not None else
    ...
    def XcalarApiFilter(self, evalString):
    ...
    def XcalarApiUpsert(self, pks, table_in=None, watermark_id='rn1'):
        ...
        # .XcalarApiIndex(keys=pks) \ (test whether excluding this index after gb works)
    ...
    def XcalarApiDeskew(self, doit=True):
        if not doit:
            return self
        self.XcalarApiRowNum('rn').XcalarApiSort({'rn': 'Ascending'})
        return self

    def XcalarApiRowNum(self, rn):
        self.query.append({
        ...
        self.query.append({
            ...
            "args": {
                ...
                "key": [{
                    "name": key,
                    "type": "DfUnknown",
                    "keyFieldName": key.split('::')[-1],
                    "ordering": order_by[key]
                } for key in order_by],
                "prefix":
                    "",
                "dhtName":
    ...
    def XcalarApiBulkLoad(self,
                          ...
                          recursive=False,
                          upperCase=False):
        dsetName = 'Optimized.{}.{}'.format(
            datetime.datetime.utcnow().timestamp(), self.dsetSeed)
        schema = [{
            'sourceColumn': c,
            'destColumn': (c.upper() if upperCase else c),
            'columnType': typeMap[self.schema[c]]
        } for c in self.schema]
        if parserArg is None:
            parserArg = {}
        ...
        # "parserArgJson": "{}" if parserArg is None else json.dumps({"opts" : parserArg}),
        ...
        # Not needed as we are always sorting by keys
        self.XcalarApiIndex(
    ...
    def XcalarApiIndex(self, keys, table_in=None, prefix=''):
        indexColumns = [{
            "name": c,
        ...
        self.query.append({
            "operation": "XcalarApiIndex",
            "args": {
                "source": table_in if table_in else self.getTempTableName(),
                "dest": self.getTempTableName(next=True),
                "key": indexColumns,
                "prefix": prefix,
                "dhtName": "",
