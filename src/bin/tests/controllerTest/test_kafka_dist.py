...
class Distributer:
    def __init__(self, xpu_count, distributer_props):
        ...
        if os.environ.get('PUBLISH_TO_KAFKA',
                          'false') == 'true' and os.environ.get(
                              'IS_INTEGRATION_TEST', 'false') == 'false':
            SingletonKafkaUtils.getInstance().publishToKafka(50)

        self.parse_args = {'maxIter': 10, 'batch_id': 0, 'metadataInfo': {}}
        self.xpu_count = xpu_count
        self.server_topic_map = SingletonKafkaUtils.getInstance(
        ).SERVER_TOPIC_MAP
        self.kafka_cluster_props = SingletonKafkaUtils.getInstance(
        ).KAFKA_CLUSTER_PROPS
