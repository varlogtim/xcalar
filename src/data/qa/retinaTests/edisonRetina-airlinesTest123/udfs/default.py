...
try:
    import xcalar.compute.udf.parquetUtils as parquetUtils
except ImportError:
    pass

import xcalar.compute.local.context as ctx
...
def xmlToJson(fullPath, xmlStream, xPath="", withPath=True, matchedPath=True):
    ...
    for path in allPaths:
        ...
        if (type(elems) == bool) or (type(elems) == float) or (type(elems) ==
                                                                     str):
            p = tree.getpath(elems)
            record = collections.OrderedDict()
            record["xcError"] = True
            record["xcErrorMessage"] = "XPath does not point to an element. Instead it is a: " + type(elems).__name__
