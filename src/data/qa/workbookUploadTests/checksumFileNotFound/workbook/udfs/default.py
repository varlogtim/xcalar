...
def xmlToJsonWithExtraKeys(fullPath, xmlStream, allPaths, withPath=True, matchedPath=True):
   tree = ET.parse(xmlStream)
   root = tree.getroot()
   tree = ET.ElementTree(root)

   for xPathStruct in allPaths:
       path = xPathStruct["xPath"]
               ...
               for extraKey in xPathStruct["extraKeys"]:
                   ...
                   if type(attributeVal) == list and len(attributeVal) > 0:
                       if ET.iselement(attributeVal[0]):
                           d[extraKey] = xmltodict.parse(ET.tostring(attributeVal[0]))[attributeVal[0].tag]
                       else:
                           d[extraKey] = attributeVal[0]
                       # Uncomment line if you want to yield all values
                       # d[extraKey] = attributeVal
               if withPath:
...
def extractJsonRecords(fullPath, inStream, structsToExtract):
    ...
    for record in jmespath.search(structsToExtract, json.loads(jsonStruct)):
        yield record
