...
ApiHandlerSessionRename::ApiHandlerSessionRename(XcalarApis api)
    : ApiHandler(api), input_(NULL)
{
}

ApiHandler::Flags
ApiHandlerSessionRename::getFlags()
{
    return (Flags)(NeedsAck | NeedsToRunImmediately);
}

Status
ApiHandlerSessionRename::run(XcalarApiOutput **outputOut, size_t *outputSizeOut)
{
    ...
    status = userMgr->rename(userId_, input_, sessionGenericOutput);
    if (status != StatusOk) {
...
Status
ApiHandlerSessionRename::setArg(XcalarApiInput *input, size_t inputSize)
{
    Status status = StatusUnknown;

    assert((uintptr_t) input == (uintptr_t) &input->sessionRenameInput);
    apiInput_ = input;
    inputSize_ = inputSize;
    input_ = &input->sessionRenameInput;
    ...
    if (input_->origSessionNameLength >= ArrayLen(input_->origSessionName)) {
        xSyslog(moduleName,
                XlogErr,
                "sessionNameLength too long (%lu chars). Max is %lu chars",
                input_->origSessionNameLength,
                ArrayLen(input_->origSessionName) - 1);
        status = StatusInval;
        goto CommonExit;
    }
