...
void
cliCastHelp(int argc, char *argv[])
{
    printf("Usage:\n\t%s <dataset> <existingColName> <newColType>\n", argv[0]);
}

void
cliCastMain(int argc,
            char *argv[],
            ...
            bool interactive)
{
    ...
    // workItem should be parsed by qpParseCastWorkItem
    ...
    if (workItem->output->hdr.status == StatusOk.code()) {
        printf("Column type changed successfully\n");
    } else {
