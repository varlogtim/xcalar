...
object TpchQuery {
  ...
  def executeQueries(sc: SparkContext, schemaProvider: TpchSchemaProvider, queryNum: Int): ListBuffer[(String, Float)] = {
    ...
    // OUTPUT_DIR without any prefixes defaults typically to HDFS on HDI spark
    // clusters on Azure
    val OUTPUT_DIR: String = "tpch-sf100-results"
  ...
  def main(args: Array[String]): Unit = {
    ...
    // INPUT_DIR without any prefixes defaults typically to HDFS on HDI spark
    // clusters on Azure. So copy a tpch-sf100 dataset to HDFS at a path which
    // will be resolved via simply "tpch-sf100" to make the following work
    val INPUT_DIR: String = "tpch-sf100"
