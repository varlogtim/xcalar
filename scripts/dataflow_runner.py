...
class DataflowRunner(object):
    ...
    def run(self, dataflowPath, params=None, dataflowName=None, exportTable=None):
        ...
        if dataflowName not in self.dataflows:
            ...
            self.dataflow.add(dataflowName, dataflowContents)
            self.dataflows.add(dataflowName)

        try:
            dataflowParameters = []
            if params:
                for key in params.keys():
                    dataflowParameters.append({PARAM_NAME : key, PARAM_VALUE : params[key]})
            ret = None
            if exportTable:
                self.dataflow.execute(dataflowName, dataflowParameters, exportTable)
                output = self.xcalarApi.listTable(exportTable)
                ret = output.nodeInfo[0].name
            else:
                ret = self.dataflow.execute(dataflowName, dataflowParameters)
        except:
